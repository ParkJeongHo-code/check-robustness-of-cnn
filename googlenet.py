# -*- coding: utf-8 -*-
"""
Created on Sun Jun 27 14:37:51 2021
...
#googlenet
def build10(input_shape,classes):
    ...
    x3=tf.keras.layers.Conv2D(32,(1,1),padding='same',activation='relu')(x2)
    x4=tf.add(x3,x)   
    ...
    x13=tf.keras.layers.Conv2D(32,(1,1),padding='same',activation='relu')(x12)
    x14=tf.add(x13,x)    
    ...
    x17=tf.keras.layers.Conv2D(32,(1,1),padding='same',activation='relu')(x16)
    x18=tf.add(x17,x14)
    ...
    x23=tf.keras.layers.Conv2D(32,(1,1),padding='same',activation='relu')(x22)
    x24=tf.add(x23,x)
    ...
    x27=tf.keras.layers.Conv2D(32,(1,1),padding='same',activation='relu')(x26)
    x28=tf.add(x27,x24)
    ...
    x31=tf.keras.layers.Conv2D(32,(1,1),padding='same',activation='relu')(x30)
    x32=tf.add(x31,x28)    
...
model_googlenet_ext=build10(input_shape=(255,255,3),classes=4)
model_googlenet_move=build10(input_shape=(255,255,3),classes=4)
model_googlenet_ori=build10(input_shape=(255,255,3),classes=4)


optimizer=optimizers.Adam()
model_googlenet_ori.compile(optimizer=optimizer,
              loss='sparse_categorical_crossentropy',
              metrics=[tf.keras.metrics.SparseCategoricalAccuracy()])

batch_size=32
epochs=15
model_googlenet_ori.fit(ori_train_data,ori_train_label,batch_size=batch_size,epochs=epochs,validation_data=(ori_val_data,ori_val_label),callbacks=[tf.keras.callbacks.EarlyStopping(patience=2)])
loss,acc=model_googlenet_ori.evaluate(ori_test_data,ori_test_label)
print("ori data test acc:%f"%acc)
os.chdir("\\home\\bmllab\\bml_pjh\\project\\for paper\\model\\pred_model")
model_googlenet_ori.save("googlenet_#1.h5")


model_googlenet_ext.compile(optimizer=optimizer,
              loss='sparse_categorical_crossentropy',
              metrics=[tf.keras.metrics.SparseCategoricalAccuracy()])


model_googlenet_ext.fit(ext_train_data,ext_train_label,batch_size=batch_size,epochs=epochs,validation_data=(ext_val_data,ext_val_label),callbacks=[tf.keras.callbacks.EarlyStopping(patience=2)])
loss,acc=model_googlenet_ext.evaluate(ext_test_data,ext_test_label)
print("ext data test acc:%f"%acc)
os.chdir("\\home\\bmllab\\bml_pjh\\project\\for paper\\model\\pred_model")
model_googlenet_move.save("googlenet_#2.h5")

model_googlenet_move.compile(optimizer=optimizer,
              loss='sparse_categorical_crossentropy',
              metrics=[tf.keras.metrics.SparseCategoricalAccuracy()])


model_googlenet_move.fit(move_train_data,move_train_label,batch_size=batch_size,epochs=epochs,validation_data=(move_val_data,move_val_label),callbacks=[tf.keras.callbacks.EarlyStopping(patience=2)])
loss,acc=model_googlenet_move.evaluate(move_test_data,move_test_label)
print("move data test acc:%f"%acc)
os.chdir("\\home\\bmllab\\bml_pjh\\project\\for paper\\model\\pred_model")
model_googlenet_move.save("googlenet_#3.h5")
